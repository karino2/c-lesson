...
- Cコンパイラの吐くアセンブリ言語を調べる事でC言語の理解を深める(これは第三回にしました)
...
以下Windows 10のWSLを前提にします。昨今ではたいていの環境で似たり寄ったりのパッケージシステムがあるでしょう。GCPなどで用意するなら、Ubuntuベースだとだいたい互換性があるはずです。
...
ではhello_arm.sを読んでいく事で、アセンブリ言語の説明をしていきたいと思います。
...
hello_arm.sで最後に残ったのが、str命令です。例えば以下みたいな行があります。
...
この、中括弧で囲む`[r0]`とか、さっき出てきた`#0x68`とかは、他にも様々な書き方でいろいろな指定が出来ます。
ですがその分複雑で、アセンブリ言語の他の要素も理解したあとじゃないと理解出来ない事も多く出てきます。

そこで、こういうのは最初のうちは出てきた物だけ覚えて、あまり最初に系統立って理解しようとしない方が良い。
ある程度慣れてきたら、その時にあらためて全ての指定方法を覚えるのがおすすめです。
...
r0が指すアドレスに、「0x68を書き込み、0x65を書き込み、0x6cを書き込み、0x6cを書き込み、00x6fを書き込み、0x0Dを書き込み、0x0Aを書き込む」訳です。

あとはアスキーコードの知識となります。第一回でパーサー書いた時に調べたでしょうか？（私は調べてません）

確か順番に'h' 'e' 'l' 'l' 'o'と、0x0D, 0x0Aを書いているのでしょうね。
...
さて、hello_arm.sを極めた我々はアセンブリマスターとしてtwitterでイキる事が出来るようになりました。
...
で、一行目はhello_arm.sと同じコードになっていますね。r3の内容を、r0の指すメモリアドレスの中身として書き込む。
...
この後に続く、ループから抜けた後のコードはhello_arm.sと同じなので説明は不要でしょう。

以上で、文字列を一文字ずつ出力する、というコマンドを説明しました。

- .ascizで文字列を埋め込む。その前にラベルをつけてそのアドレスを使う。
- addは二番目と三番目を足して一番目に入れる
- cmpは一番目と二番目を比較して、等しければ隠れたフラグ用のレジスタに入れる
- bneは隠れたフラグ用のレジスタが0だったらジャンプする

このようにcmpとbneを使ってループを作れます。addを使ってインデックスを進められます。

なお、隠れたフラグ用のレジスタをCPSRレジスタと言います。Current Program Status Registerの略らしいです。私は毎回この名前忘れてハード屋に聞いているくらいなので、この文書でもCPSRとかは言いません。
...
例えばhello_arm.binを渡すと、以下の図のようになります。

![hello_arm.binがロードされた状態](bin_load.jpg)
...
TODO: この辺でr14に8くらい足して呼び出し元の次の行をスキップして戻るような課題も追加。誰か作って。
...
普通はスタックに物を詰めたらposを進めたいのでエクスクラメーションマークつける。
...
e5 9f 00 38
e 5 9 f 0 0 3 8
1110 0101 1001 1111 0000 0000 0011 1000
```
...
|XXXX |01| 0| 1 |1001 |1111 | 0000  |    0000 0011 1000 |
...
「ベースレジスタはr15, 結果を入れるレジスタはr0, オフセットは0x38」
...
今回はベースとなるレジスタがr15、オフセットは0x38だったので、つまりr15+0x38のアドレスから読み出す事になります。中括弧表記で書くと以下です。
...
... 中略 ...
  ...
  3c:   eafffffe        b       0x3c
  40:   101f1000        andsne  r1, pc, r0
```

一行目が問題のldrです。この0x40だけ先にある命令は最後の行の以下になっています。
...
まず、2の補数表現を理解するために、具体的な例として3bitでの数3(=011)の2の補数を考えてみます。
2の補数の定義としては、2^3（これは3bitから1bit余分に追加した4bitの1000）から3を引いて、得られた結果から追加した余分なbitを捨てるというものになります。

```
2^3-3 = 1000 - 011 = 0101 -> 101
```

こんなものを考えて何が嬉しいのかというと、元々の3とその2の補数を足すことで、011 + 101 = 1000 -> 000が得られるので（こちらも追加した余分なbitを無視する）0という結果となることです。
これは3-3=0と読むことができるので、3bitでの2の補数表現において011は3で101は-3と解釈することができます。
すなわち、補数表現によって負の数も扱えていて、引き算は足し算として実現できます。

3bitでの2の補数表現を全て挙げておくと、{000(=0), 001(=1), 010(=2), 011(=3), 100(-4), 101(-3), 110(-2), 111(-1)}となります。
000はその2の補数が000なので0であることはいいとして、100もその2の補数が100だから0じゃないのか、という疑問が生じるかもしれませんが、これは-4として扱うことで010 + 100 = 110などが自然に成立します。
ここまでの話から、最上位bitを符号の情報を含む符号bitとして扱えることも理解できます（0の場合は正、1の場合は負）。

さて、2の補数表現を理解したのでジャンプ先のアドレスに戻ります。
2bitシフトしたものは
...
であり、ここでは24bitの2の補数表現を使うという約束をしているため、符号bitが1であるこいつは負の数です。
これと足すことで0が得られる24bitの数は

```
0000 0000 0000 0000 0000 1000
```

であり、これは8なので、ジャンプ先のアドレスは-8となることが理解できます。
...
cl_get_all_result()はそこまでに出力したものを、すべてつなげた文字列として取れます（間のヌル文字は取り除かれてjoinされたものが返ります）。
...
各命令について、hello_arm.sで使っているのだけを出力するのを最初の目標にする。
...
failさせるmovの他のテストのデータとしては、まずはhello_arm.binに出てくるデータだけにしておきます。
...
とりあえずhello_arm.sで使ってる奴だけを実装していきましょう。
するとあとはstrとldrでしょうか。
これらの命令も、使ってる機能だけ実装します。

特にldrは一回しか使ってなかったはず？

という事でldrはハードコードで一瞬で実装出来てしまうでしょう。

とにかく、最初はなるべくhello_arm.sを動かす最低限の実装で先に進むのを目指します。
後述するhello_arm.binのregression testを足す所までは、ひたすら手抜きでハードコードで進めます。
...
ここまででhello_arm.sで使っている物は全部実装出来たはずです。
...
一度でも良く分からないワードが来たら、そこから先は何かのデータが埋め込まれてると判断し、以後は何にも考えずに16進ダンプを最後までし続けます。
...
つまり、 test/test_input/hello_arm.bin を食わせたら test/test_expect/hello_arm.txtと同じ結果になるのが期待値。
...
hello_armのexpectくらいこっちで用意したいですが、まだ無いので最初誰かが作ったらそれ取り込みます（そればっか…）
...
hello_arm.binはほとんどハードコードで対応出来てしまったはずです。
...
この時hello_arm.binのregression testは壊さないように。
...
ただしobjdumpの出力を再現することが目的ではないということに注意してください。
objdumpはバイナリから逆アセンブルするものですが、同一のバイナリが複数のニモニックに対応し得ます（例えばpushとstmdb r13!など）。
我々は正解である元のアセンブリファイルを知っているので、元のアセンブリファイルを再現するように逆アセンブラを作っていきましょう。
具体的な例を一つ挙げると、ldmia命令がobjdumpでldmfdと表示されますが、これはldmiaと逆アセンブルするようにしてください。
...
リロケーション無しで簡単なアセンブリをアセンブルするアセンブラを作る。0x00010000決め打ち。
文字列埋め込みは最後限定で。
ラベルはサポートする。

ldrで=で大きな数字を読み込んだりはサポートしないが、ラベルで同じ事が出来るようにはする。

まずはhello_arm.sと同等の機能をアセンブル出来るアセンブラを目指します。
...
こういう時に持って来やすいコードは再利用しやすいコードと言えますが、それはゴテゴテしたコードとはだいぶ違うというのがやってみると分かると思います。

### 一行ずつ読んでasm_oneを呼んでいくmainを書く
...
hello_arm.sを動かす為には、UARTのアドレスをレジスタに読み込む方法があります。
...
まずは数字だけサポートしてみましょう。
数字は16進表記だけで。
数字はいつも4バイトとして埋め込むとします。

独自命令を作るのでアセンブリのファイルの拡張子は変えておきましょうか。
...
### hello_arm.ksを書いてアセンブルしてみる

たぶんこれくらいでhello_arm.s相当の物は実装出来るはず。
...
答えを見た後でも構わないので、いったんノートとボールペンかなんかで、このページを視界から外して文字列のケースを考えて自分で書いてみる。
...
くらいが動くように。
なお、4バイト区切りになってない端数の部分は0を埋めちゃって良いです。
ラベルをサポートする時4バイト区切りになってないと面倒だと思うので。
...
UARTのアドレスなどの数値とは処理が変わってしまうので、数値の方のラベルは対応せずにこれまで通りオフセットを手計算して凌げば良いと思いますし、
`ldr r0, =0x101f1000` 型の疑似命令に対応してしまってもいいかもしれません（どちらでもいいです）。
...
　  
昔は変なバイナリを扱う機会が多くて、そういうのを扱う人でこの辺のノウハウがある人はアセンブリ言語を作って開発するので、それをそばで見る機会は多かったので自然にみんな真似して覚えていきましたが、最近はあまりそういう機会がない。
