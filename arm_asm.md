...
で、この.globalというのはラベルをアセンブリだけじゃなくて結果のバイナリにも残して、それをリンカから見えるようにする、という感じです。
...
プログラムは実行する時はなんらかの方法でメモリにロードされて（少なくともそう見えるようにはなっていて）、
CPUはこのメモリに入っている命令の列を順番に実行していきます。

この時に次に実行する命令のアドレス、つまりメモリ上の位置を示しているのがプログラムカウンタです。

さらにこのプログラムカウンタは、mov命令などで変更する事も出来ます。

だからr15に、どこか別のアドレスをえいっと入れると、CPUはそこにジャンプして次はそこから先を延々と実行する、という振る舞いをします。
このようにして、ジャンプ、という物が実現されています。
...
TODO:あとでずれてる数字を確認。誰かprint_hexでr15出力してどれだけずれてるか教えてください。

そしてこのプログラムカウンタは、mov命令で書き換えるだけじゃなくて、読む事も出来ます。
つまり、例えば以下のコードで、現在のプログラムカウンタの値をr0に保存出来ます。

```
mov r0, r15
```

だから関数のように何か作業した後に元の場所に戻る、という場合は、
bする前にその時点のプログラムカウンタをどこかに保存しておけば良いのです。
...
この約束を守る限り、どこからprintを呼び出しても作業が終わった後に呼び出し元に戻る事が出来ます。
...
print_hex_bl.sの中身を埋めて、print_hexをblで呼べるように変更しよう。
...
コードは関数ポインタかなんか使わないと最後の++がうまい事コードを共有しつつ実装出来ないかも。
