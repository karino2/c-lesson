...
即値が終わったら、レジスタのmovも実装しましょう。
バイナリの16進とニモニック誰か教えてください。取り込みます。
...
objdumpの表示みたく、各行はアドレスから始める事にしましょう。以下みたいな感じです。
...
これらをどうC言語のプログラムに食わせるかは任せます（誰か良さそうなの書いてくれたら取り込むかも）。

helo_asmのexpectくらいこっちで用意したいですが、まだ無いので最初誰かが作ったらそれ取り込みます（そればっか…）

### regression testとUnit Test

一般的にregression testの方が全体をテストするのは簡単で、テストを用意するコストに対してバグが見つけられる費用対効果は最初のうちは高い。
...
print_loop.binとかputchar_nomem.binとかprint_hex.binとかも順番にサポートしていく。
