...
特にldrは一回しか使ってなかったはず？という事でハードコードで一瞬で実装出来てしまうしまずはその実装でテストを書くべきですが、それでテストを通した後はレジスタを変えても動くくらいにはしておきましょう。

ただし、簡易逆アセンブラでは、決め打ちな部分はある程度残ってOKです。
決め打ちだった部分と違うバイナリが出てきたらちゃんと実装する、という、必要になるまでやらない精神が大切です。
