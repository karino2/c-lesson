...
アセンブリ言語で、C言語の関数と同じ事をするコードを書いてみたいと思います。
...
branch with link、略してblというのは、現在のアドレスをr14に入れてからジャンプする、という命令です。
だから上記のb printの代わりにbl printとすると、r14にはこのblを呼んだ場所のアドレスが入ります。

で、関数は用事が終わったらこのr14に戻れば良い。

r15がプログラムカウンタなので mov r15, r14とすれば良いのを真面目に解説する。

blを使います。

メモ：リンクレジスタはr14。pcはr15



## スタックを使ってローカル変数もどきを作ろう

blだけでは、ネストした呼び出しに対応出来ません。
そこでネストした例を考えてみます。

ここでは、一文字出力するputcharと、それを使ったprint2を作ってみます。

### リンクレジスタの退避

bl print2したあとにprint2からbl putcharすると、r14がprint2のアドレスになってしまい、
print2を呼び出したアドレスが上書きされて消えてしまう。

だからbl putcharする前にr14を退避しないといけない。

これは第一回でやったXX, YY, ZZ呼び出し問題と同じで、スタックが必要。
