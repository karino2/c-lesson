...
自由に決めてもらって良いのですが、案というかこんなのでどうでしょう？という事を書いておきます。

asm_で始めて、あとはサブセクションの数字でどうでしょう？
例えばprint_hexなら、asm_2_3_print_hexなど。

別に他のラベルでも構わないのですが、ローマ字順でソートした時に第一回と区別出来る方が見る側の自分としてはありがたいです。
...
すると、だいたい以下みたいな処理でこの処理が行えそうです。（擬似コードなので実際はもっとちゃんと書きます）

```
parse_left_sbracket()
parse_register()
parse_comma_or_right_sbracket()
if (カンマなら) {
    parse_immediate()
...
bのラベルが実装出来たので、次はhello_loop.s相当の物をサポートする。
