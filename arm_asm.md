...
で、一行目はhello_asm.sと同じコードになっていますね。r3の内容アドレスr0の中身として書き込む。
...
その次にcmpというのが出てきます。これは一番目と二番目の引数を比較し、等しければ1をレジスタ（TODO：忘れた。後で調べる）に入れます。
このレジスタはcmpなどの比較演算で結果を入れる所として使われる為、あまりそれ以外の用途では使わないように心がける方がコードは読みやすいと思います。

そして次のbneは、条件付きジャンプです。条件を満たしていたらジャンプします。
その条件とは、このレジスタ（TODO）が1でない、という物です。
つまりこのレジスタが1でなければジャンプする、そうでなければジャンプしない。
第一回で作ったjmp_not_ifと同じような物ですね。（むしろあちらをbneを参考に考えたのだけど）

cmpの後に特に変な事をしなければ、cmpの結果がnot equalだったらbranchする、という事ですね。
branch not equal、つまりbne。
