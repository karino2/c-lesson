...
だからここに入ってしまわないように、手前で無限ループして止める必要があります。
...
読み込んだ一文字が0以外だったらジャンプする訳ですね。0は文字列の終わりを表しているので、文字列の終わりだったらジャンプせずに次の命令に行く、つまりループを抜けます。
0以外の普通の文字だったらloopに飛ぶ。
...
bneでジャンプせずにループから抜けると、後のコードはhello_asm.sと同じなので説明は不要でしょう。
...
まずは16進ダンプするコードを書きます。
バイナリエディタと同じような表記にして、左側にアドレス、右側に1バイトずつの16進表記を表示します。
odみたいな奴。
...
ローマ字かアンダースコアで始まり、文字っぽいものでカッコっぽく無い物が続く物。
数字とローマ字とアンダースコアは最低限許容する。

### カンマ、空白と改行

アセンブリは改行が意味を持つので改行も別トークンにしておく。
...
をアセンブルする。インターフェースとしては

```
struct Emitter;

void emit_byte(struct Emitter* emitter, char onebyte);
void emit_int(struct Emitter* emitter, int oneword);
```

くらい。Emitterは第一回を参考に。とりあえずグローバル変数に100KByteくらいの配列とってそこに詰めていく。

### aseemble関数を作る

evalに相当する関数。
トークンを読んで、先頭がmovだったらmovのバイナリを作って配列に詰める、というのを作る。

### デバッグ用に16進ダンプを表示する

ディスアセンブラからコード持ってきて16進ダンプ表示する
ここで最低限のアセンブラが出来る。
...
.rawという疑似命令を作る。
これは引数の文字列、またはint値をその場に埋め込む、という命令。
この後には.rawとラベル以外の命令は来ない物とする。
...
0xで始まる数字も即値とは別にサポート。別のトークンにしておく。
パース結果はintで。
...
ldrのオフセットは間違ってるかも。
まだビジーループが実装出来ないので0x101f1000を命令として実行しちゃいますが。
...
なおトークンとしてはコロンは取っておく。タイプのenumで区別。
...
をサポート。トークン的にはイコールとラベルを合わせてREF_LABELとかいうトークンにする。
どういうステートマシーンになるか解説する。
