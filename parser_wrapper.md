...
ループを実装する為にはまた別の通知をeval側にする必要があります。

ここでは、以下のようなAPIを実装しましょう。


```
void request_call_me_again();
```

これを呼ばれたら、そのcfuncの実行はまだ完了してない、だからevalはこのcfuncをもう一回実行しなくちゃいけない、という意味だとします。

これは前述のrequest_executeと組み合わせて使います。
この2つの関数が呼ばれると、eval_exec_arrayが実行する順序としては

1. request_executeで指定された実行可能配列を実行する
2. cfuncをもう一度実行する

という順番にする事にします。

1ではさらにこの中から終わらないcfuncが実行され得るので、このcfuncをどう覚えておくか、という問題が発生します。
これはoperation_posを一つ前に戻す事で次回継続がここに戻ってきた時にまた同じcfuncを実行するようにする、という事で対応してみましょう。

これだとeval_exec_tokenでは問題無いのですが、operation_posを使わないevalではうまく動かない事になります。
ですがevalはいつもトップレベルでだけ実行されて再帰しないので、ローカル変数で覚えておけばいいでしょう。

これで仕組みとしてはrepeatもwhileも実装出来ると思いますが、これらの関数が使う為の変数を用意する必要がありそうですね。
これは辞書に_で始まる適当な変数を作っていくとかで良さそうな気がします。適当なグローバル変数のカウンタを使って、"_0", "_1", "_2"みたいな変数を作っていくイメージです。
毎回新しい変数が必要になるつどどんどん大きな数字にしていく事で前とぶつからないようにします。
使い終わったら辞書から削除します。

ここはかなり難しいと思うので質問してください。
