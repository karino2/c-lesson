...
さて、これを呼ばれたら、そのcfuncの実行はまだ完了してないで、もう一回実行しなくちゃいけない、という意味だとします。

これは前述のrequest_executeと組み合わせて使われます。
すると順序としては、
...
という順番になる事にします。
1ではさらにネストされ得るので、このcfuncをどう覚えておくか、という問題が発生しますが、
これはoperation_posを一つ前に戻す、という事で対応してみましょう。

これだとevalではうまく動かない事になりますが、evalは再帰しないので、ローカル変数で覚えておけばいいでしょう。
