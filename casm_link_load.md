...
これは文字列定数などを置いておく所です。あとで登場するので頭の片隅に入れておいてください。
さらに、ここに述べてないものもいろいろあります。
そういうのは私も良く知らないので、ちゃんと詳しく理解したくなったら調べたらいいでしょう。
...
readelfは大きくオブジェクトファイルを調べるオプションと実行ファイル（a.outとか）を調べるオプションがあります。

まずオブジェクトファイルのセクションの情報を見るのは-Sオプションです。
...
```
$ arm-linux-gnueabi-objdump -S many_sy
mbols.o
...
Ndxという所に種類が入っているようで、初期化してないグローバル変数はCOM、未解決のシンボルはUNDと書いてありますね。
...
g_large_bufにはサイズが入っています。これは配列の大きさですね。4バイトのintが、1024*1024だけある。
...
Flgという所を見ると、R Eというのはread onlyで実行可能、RWは読み書き両方出来る、という意味です。
...
テキスト領域の先頭が0x00010000だと決める事は出来るかもしれませんが、
自分のコードがこの先頭からどこらへんに配置されるかは、他のコードがその前にどれだけ置かれるかが決まってからじゃないと分かりません。
それはリンク時に決まります。

だからオブジェクトファイルの時点では、自分の参照しているグローバル変数だけの事を考えて、それがアドレス0に仮にロードされる、という前提でバイナリを生成します。
例えば、g_var1をldrする時は\[0x00000\]、g_var2をldrする時は\[0x00000+4\]という風になっています。

リンカは各オブジェクトファイルのシンボルテーブルを見て、全グローバル変数を一列に並べて、これをどこに配置してもらうかをelfファイルに書いて、すべてのグローバル変数を参照している部分をこの「配置してもらうアドレス」+「オフセット」に直していきます。

これは第二回で、簡易アセンブラでラベルの解決をした時と似た処理になります。
線形リストで解決が必要な所を覚えておいてあとからbなどを書き直したと思いますが、あれと同じ感じで、テーブルに書き換えが必要な部分を書いておいて、最後にそれをなめて解決していきます。
...
その時に、bssというのは初期化されない変数なので、実際にはメモリを割り当てさえしてやれば、中身を何かの値にする必要はありません。
...
別にファイルに入ってなくても、ロードする時に1024*1024の、全部0で埋めた領域だけ用意してやれば良い。
...
ロードする時はファイルの該当するセクションを、メモリの特定のアドレスにロードする。
...
この時引数で -Ttextというオプションを指定して、テキスト領域を0x00010000という事でリンクを行う事を指定しています。
このバイナリではセクションは一つしか無いので分かれている必要は無いし、結果もbinに直接吐けば良いはずですが、使っているldなどのコマンドがもっと一般的なケースを基本に動く為、textセクションしか無いバイナリ、という事にして生成しています。
...
アセンブラとは、アセンブリ言語からオブジェクトファイルを作るものです。
そしてオブジェクトファイルとはおおざっぱには
...
ARMのアセンブリ言語では、セクションを表すのに、疑似命令の.textとか.dataを使います。
.textと書いてある行があると、そこから先はtextセクションに置く、という前提で処理され、.dataと書いてある行があるとそこから先はdataセクションに置くという前提で処理されます。

many_symbols.cをコンパイルしてみてみましょう。

```
arm-linux-gnueabi-gcc -S many_symbols.c
```

こうして生成されるmany_symbols.sを見ると、ごちゃごちゃとなんだかわからない物がいっぱい書かれていますが、注意してみると、.dataと.textというのが何か所か出てきているのが見つかると思います。
