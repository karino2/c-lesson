...
これはクラッシュした時のスタックトレースを表示したり、メモリリークを起こしているmallocを呼び出している関数を特定したりするのに便利です。
...
これはアセンブリに依存する為、実行バイナリをちゃんとclangのアセンブリから作って試す事にします。
...
なんとなくポインタをprintしたりして生成されるアセンブリのコードを読んで、だいたいa4はorg_sp-8+4、つまりorg_sp-4あたりを指している事を突き止めます。

なお、\[sp\]などのようにアドレスの先の値を読むのは、C言語に翻訳するとint*にキャストした後に*をする事に相当します。

次に、func1, func2, func3などの関数の最初のあたりのアセンブリを見てみると、どうもr11にpush直後のspを保存するように見えます。

TODO: 図

だから、func3を見るにはfunc2の時点でのr11を見れば良さそう。
そしてそれはpushしてあるので、org_spの-8したアドレスにありそう？

このアドレスの値をint*にキャストしてその実体をアスタリスクで取り出せば、func2のspが取れそうです。

試しにfunc2のfor文のループのカウントを取ってみましょう。
func2のアセンブリの先頭の方を見ると、func2の時点のr11に、16引いて4足したものか？
