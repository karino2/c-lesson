...
これはLinuxにはメモリに三つの属性があるからです。

### メモリの属性と実行可能領域

メモリにはR, W, Eの三つのフラグの属性がある。
...
関数ポインタのシンタックスはややこしいけれど、関数の通常の宣言の、関数の名前の所を括弧でくくって先頭にアスタリスクつければよい。
キャストは関数ポインタの宣言に変数名の所を抜けば良い。
解読するのはちょっと骨だが書くのは慣れればすぐ書けるはず。
