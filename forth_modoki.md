...
こうすると、さらに条件分岐はジャンプで実装する方が都合がよくなるので、
コンパイルも少し高機能にしてみましょう。
...
さて、ここまででとりあえず実行可能配列を一回実行すれば良い物に関しては、eval_exec_array内に展開すれば実装出来る事は分かりました。
ですが、ループはそうはいきません。
特にwhileは、条件部分を実行してからその結果に応じてbody部を実行するので、eval_exec_array内に実装するのはかなり複雑になってしまいます。

そこでループなどは、それらを実行出来るようなより簡単な命令に変換するのが良い。
具体的にはジャンプ命令です。

whileに移る前に、まずはジャンプ命令を作ってそれでifelseを置き換えていく事を考えます。
...
そこで、実行可能配列を作る時には、このような変換をすれば、eval_exec_arrayの処理を減らす事が出来ます。
この変換はどこでやるか？というと、compile_exec_arrayでやるのが良いでしょう。

### コンパイラを拡張する

コンパイラの中で実行可能ワードをコンパイルする為には、
各ワードの実装を関数ポインタにして辞書に登録するのが良いでしょう。

この関数ポインタからは、compile_exec_arrayのローカル変数のElementの配列に、
要素を追加していける必要があります。

本来は自動で伸びていくようなベクトル的なデータ構造を作る所ですが、
ここでは手抜きとして、APIだけはそういうふうに直せるようにしておきつつ、
実体はただハードコードした配列に詰めていくだけ、という体裁にしておきましょう。
...
posは各ワードの中でどれだけ追加したかを知る為にも必要です。


そして辞書をもう一つ作り、その関数ポインタはこのEmitterを受け取るようにする。

```
void (*compile_op)(struct Emitter *emitter);
```

compile_exec_arrayでは、実行可能ワードの時は辞書をlookupしてみて、
もし存在したらそれを呼ぶようにする。無ければこれまで通り実行可能ワードとして詰める、という事をします。

こうしておけば、将来ローカル変数の配列に詰めるという形式をやめて動的に伸ばしていくようなデータ構造に変えたとしても、compile_op側には変更が要りません。

YAGNIの法則からするとたぶん変更はしませんが。
...
さて、ここまで準備出来れば、whileの実装は別段難しくないでしょう。

jmpとjmp_not_ifがあれば実装出来そうです。
例えば以下のようなコード、
...
コンパイル時にこれらをemitするようにしましょう。

ここでjmp_not_ifやjmpの引数はその1つ目の数字になる訳です。
PostScriptではこうやって引数のある関数呼び出しを実現する訳ですね。

基本的に、上記の0行目と4行目以外は毎回同じで、
コード上は0行目と4行目は実行可能配列のポインタに過ぎないので、
この変換は見た目ほど難しくは無いと思います。
...
さて、最後にrepeatが残るのですが、これはローカル変数が必要でちょっとPostScript的では無い。
...
という実行可能配列をmallocで作ってこれを実行するようにしましょう。
その為にはrepeatもOP_REPEATにしてしまうのが良さそうです。

eval_exec_arrayの中でこれを実行するのはちょっとうんざりですが、ElementArrayを生成する所を関数に切り出せば、まぁいいか、くらいにはなると思います。

ちょっとひどい実装ですが、repeatはなかなか奥が深いので、今回はこの位にしておきたいと思います。
...
やはりこの位のサイズのプログラムを書くと、こうした機能の意味なども分かってくるんじゃないでしょうか。

mallocなども入門書を読むだけだと良く分からないと思います。
...
だいたい同じような構造になります。
そうしたプログラムなら割とスラスラ書けるようになっていると思います。

一応C言語の入門書は終えたが本格的なプログラムは書いた事無い、
という人でもやれるくらいを目指して作りました。12の継続のあたりだけはちょっとその範囲を超えているかもしれませんが。
...
ただ今回作ったeval_exec_arrayなどは、文字列比較さえなくせばとても小さないプログラムにする事が出来て、
...
またJavaバイトコードはよく出来ているので本格的なVMを勉強したい時にはおすすめの題材です。
