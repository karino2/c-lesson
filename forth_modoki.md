...
ソースコードとしてはこのElementArrayのポインタをunionに持たせれば話は終わりなのですが、以下C言語の勉強という事で、良い機会ですので注意すべき補足を書いておきます。
...
### 静的な型としては配列の前までしか無い事からくる制約

ElementArrayの型には、配列部分の長さは含まれていません。

だから例えば、構造体の代入はうまく動かない。
...
なお、今回のPostScriptの実装という点ではElementArray自身のコピーをする場所は無く、あくまでそれを保持したElementのコピーしか出番は無いはずなので問題ありません。
知識として知っておけば十分でしょう。
...
arrs[1]はarrsにsizeof(struct ElementArray)だけ足したアドレスを指すので、0番目のelementsの領域を指してしまう事になります。
だからこういう事は出来ません。
