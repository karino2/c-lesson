...
もし存在したらそれを呼ぶようにする。
Emitterはとりあえずローカル変数のElementの配列をstruct Emitterにセットしただけにする。
これだとオーバーランさせようと思えば簡単にしてしまうが、まぁ手抜きという事で。
...
さて、jmpとjmp_not_ifがあればループが実装出来そうです。
例えば
