...
さて、これはC言語の再帰呼び出しを使っていると自然に動いてしまうと思いますが、
そこで何が起こっているか、というのはC言語のスタックフレームの理解を必要とします。
C言語のスタックフレームを理解する為に、まずはPostScriptの世界で何が起きているのかを考えてみましょう。

ここでPostScriptの世界でXXを実行する時に何が起こるかを考えてみましょう。まず1をスタックにプッシュし、次に2をスタックにプッシュする。
...
さて、これらのC言語のスタックフレームで処理している事は、自分のスタックであらわに実装する事もできます。

今回のレッスンとしてはC言語を学ぶのが主体で言語処理系を学ぶのが主体では無いのでそれで構いません。
ですが、言語処理系を本格的に作るなら、スタック構造はホスト言語では無くあらわに持っておく方がいろいろ都合が良い事が多いのです。
