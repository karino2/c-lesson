...
1も2も両方ありますが、この場合は2の方が効率も良いしC言語っぽいので2でいきましょう。

## 可変長のarrayを使って連続領域に長さと構造体の配列を埋め込む

C言語では配列の長さ、というのは以下のように最後に構造体の配列をサイズ0で埋め込むのがイディオムとなっています（なお、古いコンパイラ向けにサイズを1にしているコードも見かけるかもしれません）。
...
構造体の型としてはelements[1]などとするとエラーになりそうな物ですが、
C言語は確保した領域さえあれば実行時に宣言した時より先の所に触っても文句を言わない仕様になっています。

そこでこうやって先頭に長さを持たせて、その次に配列の型を書いておいてmallocの時に要素分だけ多く確保するのがイディオムとなっています。

ソースコードとしてはこのElementArrayのポインタをunionに持たせれば話は終わりなのですが、以下注意すべき補足を書いておきます。
...
### sizeofは配列を含まないサイズになる

sizeofというのは型宣言でわかる値しか返しません。
ですから、上記の場合は実際に何をmallocしていようが、sizeofはint lenのサイズだけが返ります（厳密にはそれにアラインメントを足した値になります。64bit環境では4バイト多いはず）。
...
**アラインメントがずれるよ！**  
当初はここの項目、ポインタの勉強も兼ねてint分余計にmallocして長さを先頭に埋め込めば良いと思って解説を書いていたのですが、
書いていて64bitでアラインメントがずれる場合がある事に気づきました。(mallocしたのを4バイトだけずらしてsturct Elementのポインタにキャストしていたが、これでは8バイトアラインが保証されない！)  
　  
...
アラインメントについての解説もそれ以外の解説とレベルが揃わなくなるので深入りはしません。
お行儀の良いコードを書いてその辺の事はもう少しレベルが上がってから勉強するのが良いでしょう、という事で。  
　  
そういえば昔同じような事でアラインするmalloc作ったなぁ、と懐かしく思いましたが今勉強している人からすればどうでもいい話ですね。
{: .column}
