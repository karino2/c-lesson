...
さて、おもちゃならここまでの実装でも十分ですが、この実装は少し効率が悪い。
dict_getでstreqをエントリの数だけ実行するので、大変遅い実装となります。
勉強の目的も兼ねて、もっと普通の実装に改善してみましょう。
...
![ハッシュテーブルは配列の中に線形リストの先頭が入る](hash.png)
...
改善案としては、適当なランダムな数字、例えば179025684622をこのvalだけ右にビットローテートして1024で割る、とか、いろいろ考えられますが、今回はやりません。
...
PostScriptの辞書をサポートしたくなったらちょっとの変更で出来る程度には仕組みを理解しておこう、という背景があったりします。
{: .column}


### addやdefも辞書を通して実行しよう

addやdefも、関数ポインタを辞書に登録する形に変えましょう。
こうする事でprimitiveを追加してもevalを変更しなくて良くなります。
