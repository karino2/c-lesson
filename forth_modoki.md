...
### おまけ：ネストした呼び出しとC言語のスタックフレーム

さて、何も考えずにeval_exec_arrayの中でeval_exec_arrayを再帰呼び出ししていると、
特に何も考えなくても上記のコードは動いてしまうと思います。

第一回の内容としてはここで何が起こっているかをそれほど深く理解する必要は無いと思いますが、
第二回での内容の予習として、ここで何が起こっているかをちゃんと理解しておいてもいいかもしれません。
という事で以下はおまけとして一応書くだけは書いておきます。
良くわからなければ先に進んで、第二回で該当箇所を終えた後にまた戻ってきてみて下さい。

上記のコードでC言語側に何が起こるのか？というのは、
C言語のスタックフレームという物を理解する為の良い題材となります。
...
今回のレッスンとしてはC言語を学ぶのが主体で言語処理系を学ぶのが主体では無いので、そのまま先に進んでOKです。
ですが、言語処理系を本格的に作るなら、スタック構造はホスト言語では無くあらわに持っておく方がいろいろ都合が良い事が多いものです。

そこで以下におまけとして、このスタックをあらわに実装する方法も簡単に書いておきます。
おまけなのでやらなくてOKです。（やったら見せてくれればレビューはします）。
...
現在実行可能配列のどこを実行しているか、というのは、実行可能配列とそのElementArrayの何番目か、という位置の組になります。

この何番目かを表す物を、第二回のアセンブリの内容と近づける為、以下ではpcと呼びます。プログラムカウンタの略です。
...
この2つの情報があれば、その実行可能配列の実行を再開する事ができます。
ですが、これではその実行可能配列の実行が終わったあとに戻れない。
...
すると内側のeval_exec_arrayで全co_stackの中身が処理されて、その後外側のeval_exec_arrayに戻った時に変な状態になってしまいます。

そこで自然な処理としては、C関数のprimitivesから実行可能配列を実行したい場合は、
何らかの方法でそのcfuncを呼んでるeval_exec_array側に戻ってco_stackに継続をpushしてwhileをやり直す、という処理をする必要があります。

このcfuncとその呼び出し元をどう協業するか、というのは、いろいろ考えられますが、多少汚い実装になってしまうのも仕方ないかな、と思います。ぱっと考えられるのは、

1. primitiveの中でco_stackを更新して、更新したからwhileの先頭にもどれ、という指示を意味する戻り値を返す
2. グローバル変数などに次実行すべき実行可能配列などをこそっと保存して、cfunc呼んだ後にこれがあったらeval_exec_array側で処理する

とかそんな感じでしょうか。
こうしたCのプリミティブからeval側に戻らなくてはいけない、というのは、
外部の非同期APIにつなぐ時やyieldみたいな機能を実装する時にも発生するので、まぁまぁ良くあります。
