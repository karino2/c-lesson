...
ワードとはforthでの変数名に相当するものです。
具体例としては「abc」とか「abc123」とか「abc123+」とかで、
厳密な言い方をすると「ローマ字で始まってスペース以外が続く文字で構成された文字列」です。
簡単のため、ワードは255文字以下としましょう。
256文字以上のワードは来ないものと仮定して、エラー処理などはしなくてOKです。
最後のヌル文字も入れた256をWORD_SIZEとしてdefineしておくので、必要に応じて使ってください。

ワードは、WORDというタイプで文字列をmallocして返す事にしましょう。
...
そうで無い場合にどうするか？というのは意見が割れる所で、
C言語においてfreeをすべきかどうかは定期的に論争が発生します。　　
　　
