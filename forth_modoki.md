...
この変換はどこでやるか？実行時なら完全に上記の実行可能配列を吐く事が出来ます。
...
さて、コードの生成を実行時にするのは遅いですし、本来はcompile_exec_arrayの段階でもなんとか出来るならした方が良いです。
...
3
1
roll % cond {123} {456} を {123} {456} cond の順番にする
5
jmp_not_if
...
こうすれば、eval_exec_arrayの中ではifelseが来る事は無くなります。
...
void compile_exec_array(...) {
  ...
  ...
     ...
     if("ifelse"だったら) {
        arrayに以下のコードを詰める
             3 1 roll 5 jmp_not_if pop exec 4 jmp exch pop exec
     } else {
        arrayにそのEXECUTABLE_WORDを詰める
     }
  }
...
そこで関数に抜き出したいと思います。
名前はifelse_compileにしたいと思います。

さて、この関数にはどういう引数が必要でしょうか？

### ifelse_compileに渡すEmitterを考える
...
動的なコード生成なら、今や別段説明無しで実装出来てしまうでしょう。
...
今回もeval_exec_array内でこそっと実行してみてください。
...
こうするとbodyをexecした後のスタックの状態はwhileには分からないので、exchなどをしたとしてもexchする前においておいた物が触れるとは限りません。

そこで、どうやってもローカル変数が必要になってしまうと思います。
そこでローカル変数を実装する事にしましょう。
...
このスタックにも、Elementを詰めるようにして、現在の実行可能配列の実行が終わってco_stackからpopする時に、Continuation構造体まで全部popしていまえば良いでしょう。
...
さて、最後にrepeatが残るのですが、これは動的にコード生成でやってもあんまり楽にならない。
例えば以下のコードを
...
動的コード生成で実装出来ます。
ただループの回数に応じた長さの配列をmallocする必要があるのでいまいちですね。

普通にローカル変数を使えば、動的コード生成でも静的コード生成でもあまり大変さは変わらないと思うので、
最初から静的コード生成で実装してしまいましょう。
