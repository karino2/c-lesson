...
何らかの方法でそのcfuncを呼んでる呼び出し元のeval_exec_array側に戻ってco_stackに継続をpushしてwhileをやり直す、という処理をする必要があります。

このcfuncとその呼び出し元をどう協業するか、というのは、いろいろ考えられますが、
あとのwhileの独自拡張の実装と合わせると私が最初思ってたよりも難しい事が判明しました(^^;

詳細は結論が出たらまとめますが、現状は[parserのラッパ路線](parser_wrapper.md)の下の方を参照ください。
