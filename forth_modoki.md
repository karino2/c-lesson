...
6. Fizz Buzz。modの実装が要る。数字を順番にカウントしていくが、以下の場合だけ数字では無くて文字を出力。3と5で割れたら /FizzBuzz、それ以外で5で割れたら/Buzz、それ以外で3で割れたら/Fizz。今回は数字をスタックに積んでいって、FizzとかBuzzはリテラルネームをpushする事にしましょう。 15 FizzBuzz とやったら数字やリテラルネームを15までスタックに積んでいく事とします。
...
などです。こういう時にif側とelse側を全部書こう、と書くとテストの数が膨れ上がってしまうので、まずは最低一つのケーステストを一通り揃えます。
その後気になる所は追加していきます。


以下のテストも足しておきましょう。（やった人が結構苦戦してたので）

```
/f {{1 3 add} exec 3} def f
```

複雑な例として、factorialもUnitTestの形で追加しておきましょう。
...
なお、ローカル変数があればifelseももうちょっと素直な実装に変更出来ると思います。
