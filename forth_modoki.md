...
さて、ポインタにすると長さが分からないので長さもどうにか知る必要があります。
選択肢は2つくらいあって、

1. 最後をNULLにしておく
2. 先頭に長さを埋め込む
3. unionの中にstructをネストしてintを渡す
...
自分なら2かな。1の方が簡単なので1でも良さそう。この辺は育った環境次第な気がします。昔COMをやってた人は先頭に長さを詰めて、Unixで育った人は最後にNULLを入れる気がする。どっちでもいいです。
...
PostScriptの仕様的に、これではローカル変数にならないので変なコードを書かないとあまり出てきません。
だからサポートしない、というのも十分アリな選択と思います。

ただ、あとで出てくる条件分岐などと組み合わせて変な事をやろうとすればやれない事は無くて、
そういう変な事が出来るのはPost Scriptの楽しさのような気もするので、実装する意味が無い訳でも無い。

せっかくなので対応する事にしますか。

なお、まずはネストが無い状態で実装を終えて、ネストが無い場合のテストが通る事を確認してからネストの対応に入ってください。

といっても実装は大した事ありません。compile_exec_arrayの途中で"{"にぶつかったら、compile_exec_arrayを呼び出し、その結果をbyte_codesの配列に入れます。
...
　  
そもそもに再帰呼び出しがあれば、だいたいの場合にスタックオーバーフローをさせる事が出来るので、
真面目にやる場合には再帰呼び出し自体が最初から深さの最悪値がちゃんと決まる場合以外では使うべきでは無い、
という事になるので、スタックに配列を取るかどうかは関係ないと言えば関係ない。  
　  
...
ここでXXを順番に実行していく事を考えましょう。2をスタックにプッシュするのは良いですが、
次にYYを実行しろ、と言われます。

YYはdict_getすると実行可能配列のようなのでこれを次に実行していく訳ですが、
このYYの実行が終わった後もまだXXにはやる事が残っています。そう、3をプッシュする事です。

つまり、YYを実行する時には、それが終わった後にXXの続きをどこから再開するかを覚えておく必要があります。
下の図で言う、赤の矢印がどこに戻るかを覚えておく、という事ですね。
...
個人的には今回のシリーズなら1までで良いと思うのですが、もし興味が湧いた人の為に継続版の実装も簡単に解説しておきます。

### 継続とスタックを使った実装

実行可能配列の現在どこを実行しているか、というindexを、ここではアセンブリ言語にならってプログラムカウンタ、略してpcと呼ぶ事にします。
...
こういうふうにすると、このco_stackをどこまで巻き戻すかで非局所脱出を実装出来ますし、Cのスタックも消費しません。

まぁまぁ面倒なので実装してもしなくてもいいです。
実装する場合はcontinuation.cというファイルに実装を分けて下さい。
...
1. factorial.ps 上記のfactorialを実装し、最後に 10 factorial を実行
